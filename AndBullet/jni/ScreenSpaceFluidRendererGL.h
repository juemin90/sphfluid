...
class ScreenSpaceFluidRendererGL
{
	...
	GLuint m_blurColorShader;
	GLuint m_blitShader;

	GLuint m_gausianFilterRGBAShader;
	...
	GLuint m_surfaceBlurTexture1;
	GLuint m_surfaceBlurTexture2;

	//blur filter variable
	GLfloat m_texelSize_X;
	GLfloat m_texelSize_Y;
	GLfloat m_RadiusPixels;
	GLfloat m_Scale;
	GLfloat m_DepthFallOff;\
	GLfloat m_DepthFar;
	GLfloat m_DepthNear;
...
public:
	...
	void setCheckedBlurThick(bool isChecked) {
		isCheckedBlurThick = isChecked;
	}

	bool getCheckedBlurThick() {
		return isCheckedBlurThick;
	}
...
private:
	...
	void render_blurNormalTexture();
	void render_blurGausianNormal();
	void render_blurGausianThick();
	void render_generateNormalTexture(const btAlignedObjectArray<btVector3>& particlePositions, float sphereRadius);
...
const char generateDepthVertexShader[] =
	...
	"	pPos = pMat * mvMat * vec4(v_Pos.xyz, 1.0);\n"
	"	gl_Position = pPos;\n"
	"}";
//*/
...
//*
	...
	"	Color = vec4(depth, (normal.z/2.0f + 0.5f), pointRadius, thickness);\n"
	"	gl_FragDepth = depth;\n"
	"}";
...
/*
	...
	"	if(r2 > 1.0) discard;\n"
	"	normal.z = sqrt(1.0 - r2);\n"
	"	vec4 p_Pos = pMat * vec4(eyePosition + (normal * pointRadius), 1.0f);\n"
	"	float depth = p_Pos.z / p_Pos.w;"
	"	float thickness = normal.z * 0.03;\n"
	"	Color = vec4(vec3(depth), thickness);\n"
	"	gl_FragDepth = depth;\n"
	"}";
//*/

const char generateNormalFragmentShader[] =
//*
	"#version 300 es\n"
	"precision highp float;\n"
	"uniform mat4 mvMat;\n"
	...
	"	normalize(normal);\n"
	"	float depth = (pPos.z / pPos.w);"
	"	depth += normal.z * pointRadius * (1.0/999.0);"
	//"	normal = (pMat * vec4(normal.xyz, 0.0f)).xyz;\n"
	"	normal = (mvMat * vec4(normal.xyz, 0.0f)).xyz;"
	"	normal = (normal / 2.0) + 0.5;\n"
	"	Color = vec4(normal.x, normal.y, normal.z, depth);\n"
	//"	Color = vec4(0.0f, normal.y, 0.0f, 1.0f);\n"
	"	gl_FragDepth = depth;\n"
	"}";
...
const char bilateralFilter1dFragmentShader_depth[] =
	...
	"}";
//*/
const char bilateralFilter1dFragmentShader_color[] =
	"#version 300 es\n"
	"precision highp float;\n"
	"uniform float texelSize;\n"
	"uniform float filterRadiusPixels;\n"
	...
	"	vec4 sum = vec4(0.0);\n"
	"	vec4 wsum = vec4(0.0);\n"
	"	vec4 color = texture(colorTexture, Tex);\n"
	"	for(float x = -filterRadiusPixels; x <= filterRadiusPixels; x += 1.0)\n"
	"	{\n"
	"		vec4 neighborColor = texture(colorTexture, Tex.xy + blurDirection*texelSize*x);\n"
	"		float r = x * blurScale;\n"
	"		float w = exp(-r*r);\n"
	"		vec4 r2 = (neighborColor - color) * blurDepthFalloff;\n"
	"		vec4 g;\n"
	"		g = exp(r2 * r2);"
	"		sum += neighborColor * g * w;\n"
	"		wsum += w * g;\n"
	"	}\n"
	"	if(wsum.r > 0.0){\n"
	"		sum.r /= wsum.r;\n"
...
const char bilateralFilter1dFragmentShader_alpha[] =
	...
	"	for(float x = -filterRadiusPixels; x <= filterRadiusPixels; x += 1.0)\n"
	"	{\n"
	"		float neighborDepth = texture(alphaTexture, Tex.xy + blurDirection*texelSize*x).r;\n"
	"		float r = x * blurScale;\n"
	"		float w = exp(-r*r);\n"
	"		float r2 = (neighborDepth - depth) * blurDepthFalloff;\n"
	"		float g = exp(-r2*r2);\n"
...
const char generateSurfaceFragmentShader[] =
	//*
		"#version 300 es\n"
			...
			"vec3 getEyePos(sampler2D Texture, vec2 texCoord, mat4 projectionMatrix)\n"
			"{\n"
			"	float depth = texture(Texture, texCoord).w;\n"
			...
			"void main()\n"
			"{\n"
			"	vec3 eyePosition = getEyePos(normalTexture, Tex.xy, depthProjectionMatrix);\n"
			"	vec4 data = texture(normalTexture, Tex);\n"
			"	vec3 normal = normalize((data.xyz * 2.0f) - 1.0f);\n"
			"	normal.y  *= -1.0f;\n"
			"	float depth = data.w;\n"
			"	if(depth >= DepthRange_Far) discard;\n"
...
const char gausianFilter[] =
		...
		"  float coefficientSum = 0.0f;\n"
		 ...
		 " avgValue += texture(colorTexture, Tex) * incrementalGaussian.x;\n"
		 " coefficientSum += incrementalGaussian.x;\n"
		 " incrementalGaussian.xy *= incrementalGaussian.yz;\n"

		  // Go through the remaining 8 vertical samples (4 on each side of the center)
		 " for (float i = 1.0f; i <= filterRadiusPixels; i++) { \n"
		 "   avgValue += texture(colorTexture, Tex - i * texelSize *  blurMultiplyVec) * incrementalGaussian.x;\n"
		 "   avgValue += texture(colorTexture, Tex + i * texelSize * blurMultiplyVec) * incrementalGaussian.x;\n"
