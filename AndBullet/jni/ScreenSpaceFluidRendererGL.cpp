...
ScreenSpaceFluidRendererGL::ScreenSpaceFluidRendererGL(int screenWidth, int screenHeight)
{
	...
	m_generateNormalShader = compileProgram(generateDepthVertexShader, generateNormalFragmentShader);
	...
	m_blurColorShader = compileProgram(fullScreenTextureVertexShader, bilateralFilter1dFragmentShader_color);
	m_blitShader = compileProgram(fullScreenTextureVertexShader, blitFragmentShader);

	m_gausianFilterRGBAShader = compileProgram(fullScreenTextureVertexShader, gausianFilter);
	...
	m_normalTexture = m_frameBuffer.createFrameBufferTexture(FrameBufferGL::FBT_RGBA_TEXTURE);
	...
	m_surfaceBlurTexture1 = m_frameBuffer.createFrameBufferTexture(FrameBufferGL::FBT_RGBA_TEXTURE);
	m_surfaceBlurTexture2 = m_frameBuffer.createFrameBufferTexture(FrameBufferGL::FBT_RGBA_TEXTURE);
	...
	m_texelSize_X = 1.0f / m_windowWidth;
	m_texelSize_Y = 1.0f / m_windowHeight;
	m_RadiusPixels = 9.0f;
	m_Scale = 0.17f;
	m_DepthFallOff = 0.5f;
	m_DepthNear = 0.0f;
...
ScreenSpaceFluidRendererGL::~ScreenSpaceFluidRendererGL()
{
	...
	glDeleteShader(m_blurColorShader);
	glDeleteShader(m_gausianFilterRGBAShader);
	//
...
//*/
	...
	bool renderingResolutionDiffers = ( m_windowWidth != screenWidth || m_windowHeight != screenHeight );
	if(renderingResolutionDiffers)
		glViewport( 0, 0, screenWidth, screenHeight );

//	LOGI("Generate Normal");
	if(isCheckedBlurThick) {
//		render_stage1_generateDepthTexture(particlePositions, sphereRadius);
//		render_stage2_blurDepthTextureCurvatureFlow();
//		render_stage2_blurDepthTextureBilateral();
		render_generateNormalTexture(particlePositions, sphereRadius);
	} else {
		render_generateNormalTexture(particlePositions, sphereRadius);
	}
//	LOGI("Blur Normal");
	if(isCheckedBlurThick) {
		render_blurNormalTexture();
	} else {
		render_blurGausianNormal();
	}
//	LOGI("Generate Thick");
...
void ScreenSpaceFluidRendererGL::render_stage1_generateDepthTexture(const btAlignedObjectArray<btVector3>& particlePositions, float sphereRadius)
{
	glUseProgram(m_generateDepthShader);
	{
		...
		glUniform1f( glGetUniformLocation(m_generateDepthShader, "DepthRange"), m_DepthFar - m_DepthNear);
...
void ScreenSpaceFluidRendererGL::render_stage2_blurDepthTextureCurvatureFlow()
{
	...
	glUniform2f( glGetUniformLocation(m_curvatureFlowShader, "texelSize"), m_texelSize_X, m_texelSize_Y );
	//glUniform1f( glGetUniformLocation(m_curvatureFlowShader, "timeStep"), 0.001f );
	glUniform1f( glGetUniformLocation(m_curvatureFlowShader, "timeStep"), 0.5f );
	glUniform1i( glGetUniformLocation(m_curvatureFlowShader, "depthTexture"), 0 );

	glUniform1f(glGetUniformLocation(m_curvatureFlowShader, "DepthRange_Near"), m_DepthNear);
	glUniform1f(glGetUniformLocation(m_curvatureFlowShader, "DepthRange_Far"), m_DepthFar);
...
void ScreenSpaceFluidRendererGL::render_stage2_blurDepthTextureBilateral()
{
	...
	glUniform1f( glGetUniformLocation(m_blurDepthShader, "texelSize"), m_texelSize_X );
	glUniform1f( glGetUniformLocation(m_blurDepthShader, "filterRadiusPixels"), m_RadiusPixels);
	glUniform1f( glGetUniformLocation(m_blurDepthShader, "blurScale"), m_Scale );
	glUniform1f( glGetUniformLocation(m_blurDepthShader, "blurDepthFalloff"), m_DepthFallOff );
	...
	glUniform1f(glGetUniformLocation(m_blurDepthShader, "DepthRange_Near"), m_DepthNear);
	glUniform1f(glGetUniformLocation(m_blurDepthShader, "DepthRange_Far"), m_DepthFar);

	m_frameBuffer.attachAndSetRenderTargets(m_blurredDepthTexturePass1, m_tempDepthTexture);
		renderFullScreenTexture(m_depthTexture, 0, 0);
	m_frameBuffer.detachAndUseDefaultFrameBuffer();

	//Second pass blurs along the y-axis
	glUniform1f( glGetUniformLocation(m_blurDepthShader, "texelSize"), m_texelSize_Y );
	glUniform1f( glGetUniformLocation(m_blurDepthShader, "filterRadiusPixels"), m_RadiusPixels );
	glUniform1f( glGetUniformLocation(m_blurDepthShader, "blurScale"), m_Scale );
	glUniform1f( glGetUniformLocation(m_blurDepthShader, "blurDepthFalloff"), m_DepthFallOff );
	...
	m_frameBuffer.attachAndSetRenderTargets(m_blurredDepthTexturePass2, m_tempDepthTexture);
...
void ScreenSpaceFluidRendererGL::render_stage4_blurThickTexture()
{
	...
	//First pass blurs along the x-axis
	glUniform1f( glGetUniformLocation(m_blurThickShader, "texelSize"), m_texelSize_X );
	glUniform1f( glGetUniformLocation(m_blurThickShader, "filterRadiusPixels"), m_RadiusPixels );
	glUniform1f( glGetUniformLocation(m_blurThickShader, "blurScale"), m_Scale );
	glUniform1f( glGetUniformLocation(m_blurThickShader, "blurDepthFalloff"), m_DepthFallOff );
	...
	//Second pass blurs along the y-axis
	glUniform1f( glGetUniformLocation(m_blurThickShader, "texelSize"), m_texelSize_Y );
	glUniform1f( glGetUniformLocation(m_blurThickShader, "filterRadiusPixels"), m_RadiusPixels );
	glUniform1f( glGetUniformLocation(m_blurThickShader, "blurScale"), m_Scale );
	glUniform1f( glGetUniformLocation(m_blurThickShader, "blurDepthFalloff"), m_DepthFallOff);
...
void ScreenSpaceFluidRendererGL::render_stage6_generateSurfaceTexture()
{
	...
	glUniformMatrix4fv( glGetUniformLocation(m_generateSurfaceShader, "depthProjectionMatrix"), 1, false, (GLfloat*)projMat );
	glUniform4f( glGetUniformLocation(m_generateSurfaceShader, "Color"), m_r, m_g, m_b, 1.0f);
	glUniform2f( glGetUniformLocation(m_generateSurfaceShader, "texelSize"), 1.0f /m_frameBuffer.getWidth() , 1.0f / m_frameBuffer.getHeight());

	glUniform1i( glGetUniformLocation(m_generateSurfaceShader, "absorptionAndTransparencyTexture"), 0 );
	glUniform1i( glGetUniformLocation(m_generateSurfaceShader, "normalTexture"), 1 );

	glUniform1f(glGetUniformLocation(m_generateSurfaceShader, "DepthRange_Near"), m_DepthNear);
	glUniform1f(glGetUniformLocation(m_generateSurfaceShader, "DepthRange_Far"), m_DepthFar);

	m_frameBuffer.attachAndSetRenderTargets(m_surfaceColorTexture, m_surfaceDepthTexture);
		if(isCheckedBlurThick) {
			renderFullScreenTexture(m_absorptionAndTransparencyTexture, m_surfaceBlurTexture2,0);
...
void ScreenSpaceFluidRendererGL::render_blurNormalTexture()
{
	glUseProgram(m_blurColorShader);

	//First pass blurs along the x-axis
	glUniform1f( glGetUniformLocation(m_blurColorShader, "texelSize"), m_texelSize_X );
	glUniform1f( glGetUniformLocation(m_blurColorShader, "filterRadiusPixels"), m_RadiusPixels);
	glUniform1f( glGetUniformLocation(m_blurColorShader, "blurScale"), m_Scale );
	glUniform1f( glGetUniformLocation(m_blurColorShader, "blurDepthFalloff"), m_DepthFallOff );
	glUniform2f( glGetUniformLocation(m_blurColorShader, "blurDirection"), 1.0f, 0.0f );
	glUniform1i( glGetUniformLocation(m_blurColorShader, "colorTexture"), 0 );

	glUniform1f(glGetUniformLocation(m_blurColorShader, "DepthRange_Near"), m_DepthNear);
	glUniform1f(glGetUniformLocation(m_blurColorShader, "DepthRange_Far"), m_DepthFar);

	m_frameBuffer.attachAndSetRenderTargets(m_surfaceBlurTexture1, m_tempDepthTexture);
		renderFullScreenTexture(m_normalTexture, 0, 0);
	m_frameBuffer.detachAndUseDefaultFrameBuffer();

	//Second pass blurs along the y-axis
	glUniform1f( glGetUniformLocation(m_blurColorShader, "texelSize"), m_texelSize_Y );
	glUniform1f( glGetUniformLocation(m_blurColorShader, "filterRadiusPixels"), m_RadiusPixels );
	glUniform1f( glGetUniformLocation(m_blurColorShader, "blurScale"), m_Scale );
	glUniform1f( glGetUniformLocation(m_blurColorShader, "blurDepthFalloff"), m_DepthFallOff );
	glUniform2f( glGetUniformLocation(m_blurColorShader, "blurDirection"), 0.0f, 1.0f );
	glUniform1i( glGetUniformLocation(m_blurColorShader, "colorTexture"), 0 );

	glUniform1f(glGetUniformLocation(m_blurColorShader, "DepthRange_Near"), m_DepthNear);
	glUniform1f(glGetUniformLocation(m_blurColorShader, "DepthRange_Far"), m_DepthFar);

	m_frameBuffer.attachAndSetRenderTargets(m_surfaceBlurTexture2, m_tempDepthTexture);
		renderFullScreenTexture(m_surfaceBlurTexture1, 0, 0);
	m_frameBuffer.detachAndUseDefaultFrameBuffer();
}
void ScreenSpaceFluidRendererGL::render_blurGausianNormal(){
	glUseProgram(m_gausianFilterRGBAShader);

	//First pass blurs along the x-axis
	glUniform1f( glGetUniformLocation(m_gausianFilterRGBAShader, "texelSize"), m_texelSize_X );
	glUniform1f( glGetUniformLocation(m_gausianFilterRGBAShader, "filterRadiusPixels"), m_RadiusPixels);
	glUniform1f( glGetUniformLocation(m_gausianFilterRGBAShader, "sigma"), 5.0f);
	glUniform2f( glGetUniformLocation(m_gausianFilterRGBAShader, "blurMultiplyVec"), 1.0f, 0.0f );
	glUniform1i( glGetUniformLocation(m_gausianFilterRGBAShader, "colorTexture"), 0 );
	...
	//Second pass blurs along the y-axis
	glUniform1f( glGetUniformLocation(m_gausianFilterRGBAShader, "texelSize"), m_texelSize_Y );
	glUniform1f( glGetUniformLocation(m_gausianFilterRGBAShader, "filterRadiusPixels"), m_RadiusPixels );
	glUniform1f( glGetUniformLocation(m_gausianFilterRGBAShader, "sigma"), 5.0f);
	glUniform2f( glGetUniformLocation(m_gausianFilterRGBAShader, "blurDirection"), 0.0f, 1.0f );
	glUniform1i( glGetUniformLocation(m_gausianFilterRGBAShader, "colorTexture"), 0 );
...
void ScreenSpaceFluidRendererGL::render_blurGausianThick()
{
	glUseProgram(m_gausianFilterRGBAShader);
...
void ScreenSpaceFluidRendererGL::render_generateNormalTexture(const btAlignedObjectArray<btVector3>& particlePositions, float sphereRadius){
	glUseProgram(m_generateNormalShader);
	{
		...
		float screenHeight = static_cast<float>( m_frameBuffer.getHeight() );
		float lesserDistance = (screenWidth > screenHeight) ?  screenHeight : screenWidth;
		glUniform1f( glGetUniformLocation(m_generateNormalShader, "pointScale"), lesserDistance );
		glUniform1f( glGetUniformLocation(m_generateNormalShader, "pointRadius"), sphereRadius );
		glUniformMatrix4fv(glGetUniformLocation(m_generateNormalShader, "mvMat"), 1, false, (GLfloat*)viewMat);
		glUniformMatrix4fv(glGetUniformLocation(m_generateNormalShader, "pMat"), 1, false, (GLfloat*)projMat);

		glVertexAttribPointer(0, 4, GL_FLOAT, false, 0, &particlePositions[0]);
		glEnableVertexAttribArray(0);

		m_frameBuffer.attachAndSetRenderTargets(m_normalTexture, m_tempDepthTexture);
			glDrawArrays(GL_POINTS, 0, numParticles);
		m_frameBuffer.detachAndUseDefaultFrameBuffer();
