...
void FrameBufferGL::attachAndSetRenderTargets(GLuint out_colorTexture, GLuint out_depthTexture)
{
	...
	if(frameBufferStatus != GL_FRAMEBUFFER_COMPLETE)
		LOGI("Error: OpenGL FrameBuffer is not properly configured(%d).\n", frameBufferStatus);
...
void FrameBufferGL::resizeTextures(int width, int height)
{
	...
	for(int i = 0; i < m_depthTextures.size(); ++i)
	{
		...
		glTexImage2D(GL_TEXTURE_2D, MIPMAP_LEVEL, GL_DEPTH_COMPONENT16, width, height, 0, GL_DEPTH_COMPONENT, GL_FLOAT, 0);
	}
...
GLuint FrameBufferGL::internalCreateFrameBufferTexture(int width, int height, FramebufferTextureType type)
{
	...
	switch(type)
	{
		case FBT_R32F_TEXTURE:
			glTexImage2D(GL_TEXTURE_2D, MIPMAP_LEVEL,  GL_R32F , width, height, 0,  GL_RED , GL_FLOAT, 0);
			LOGI("R32F Buffer Creation Count: %d\n",textureId);
			break;
		case FBT_DEPTH_TEXTURE:
			glTexImage2D(GL_TEXTURE_2D, MIPMAP_LEVEL,  GL_DEPTH_COMPONENT16 , width, height, 0,  GL_DEPTH_COMPONENT ,  	GL_UNSIGNED_SHORT, 0);
			//glTexImage2D(GL_TEXTURE_2D, MIPMAP_LEVEL,  GL_RGBA8 , width, height, 0,  GL_RGBA , GL_UNSIGNED_BYTE, 0);
			LOGI("Depth Buffer Creation Count: %d\n",textureId);
			break;
		case FBT_ALPHA_TEXTURE:
			//glTexImage2D(GL_TEXTURE_2D, MIPMAP_LEVEL, GL_RGBA32F, width, height, 0, GL_RGBA, GL_FLOAT, 0);
			glTexImage2D(GL_TEXTURE_2D, MIPMAP_LEVEL, GL_RGBA8, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0);
			LOGI("Alpha Buffer Creation Count: %d\n",textureId);
			break;
		case FBT_RGBA_TEXTURE:
		default:
			glTexImage2D(GL_TEXTURE_2D, MIPMAP_LEVEL, GL_RGBA8, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0);
			//glTexImage2D(GL_TEXTURE_2D, MIPMAP_LEVEL, GL_R32F, width, height, 0, GL_RED, GL_FLOAT, 0);
			LOGI("RGBA Buffer Creation Count: %d\n",textureId);
			break;
	}
