...
void btFluidSphSolver::applyForcesSingleFluid(const btFluidSphParametersGlobal& FG, btFluidSph* fluid)
{
	...
	for(int i = 0; i < particles.size(); i+=4)
	{
		...
		btVector3& vel1 = particles.m_vel[i];
		btVector3& vel_eval1 = particles.m_vel_eval[i+1];

		btVector3& vel2 = particles.m_vel[i];
		btVector3& vel_eval2 = particles.m_vel_eval[i+2];

		btVector3& vel3 = particles.m_vel[i];
		btVector3& vel_eval3 = particles.m_vel_eval[i+3];
		...
		vel_eval1 = vel_eval;				//v(t+1) = [v(t-1/2) + v(t+1/2)] * 0.5		used to compute (sph)forces later
		vel1 = vel;

		vel_eval2 = vel_eval;				//v(t+1) = [v(t-1/2) + v(t+1/2)] * 0.5		used to compute (sph)forces later
		vel2 = vel;

		vel_eval3 = vel_eval;				//v(t+1) = [v(t-1/2) + v(t+1/2)] * 0.5		used to compute (sph)forces later
		vel3 = vel;
...
void btFluidSphSolver::integratePositionsSingleFluid(const btFluidSphParametersGlobal& FG, btFluidParticles& particles)
{
	...
	for(int i = 0; i < particles.size(); i+=4) {
		btVector3 move = particles.m_vel[i] * timeStepDivSimScale;
		particles.m_pos[i] += move;
		particles.m_pos[i+1] += move;
		particles.m_pos[i+2] += move;
		particles.m_pos[+3] += move;
	}
}
...
void btFluidSphSolverDefault::sphComputePressure(const btFluidSphParametersGlobal& FG, btFluidSph* fluid, btFluidSphSolverDefault::SphParticles& sphData)
{
	...
	{
		BT_PROFILE("sphComputePressure()-reset_sums_and_clear_table");
	...
	{
		BT_PROFILE("sphComputePressure()-compute_sums");
	...
	
	{
		...
		for(int i = 0; i < numParticles; i+=4)
		{
			...
			btScalar stiffness = (density - FL.m_restDensity) * FL.m_stiffness;
			btScalar rev_density = btScalar(1.0) / density;

			sphData.m_pressure[i] = stiffness;
			sphData.m_invDensity[i] = rev_density;

			sphData.m_pressure[i+1] = stiffness;
			sphData.m_invDensity[i+1] = rev_density;

			sphData.m_pressure[i+2] = stiffness;
			sphData.m_invDensity[i+2] = rev_density;

			sphData.m_pressure[i+3] = stiffness;
			sphData.m_invDensity[i+3] = rev_density;
		}
	}
}
...
void btFluidSphSolverDefault::sphComputeForce(const btFluidSphParametersGlobal& FG, btFluidSph* fluid, btFluidSphSolverDefault::SphParticles& sphData)
{
	...
	for(int i = 0; i < particles.size(); i+=4){
		...
		sphData.m_sphForce[i+1] = sphData.m_sphForce[i];
		sphData.m_sphForce[i+2] = sphData.m_sphForce[i];
		sphData.m_sphForce[i+3] = sphData.m_sphForce[i];
	}
}
