...
protected:
	static void applySphForce(const btFluidSphParametersGlobal& FG, btFluidSph* fluid, const btAlignedObjectArray<btVector3>& sphForce)
	{
		...
		for(int n = 0; n < fluid->numParticles(); n+=4)
		{
			...
			fluid->applyForce(n+1, acceleration * FL.m_particleMass);
			fluid->applyForce(n+2, acceleration * FL.m_particleMass);
			fluid->applyForce(n+3, acceleration * FL.m_particleMass);
		}
	}
};
